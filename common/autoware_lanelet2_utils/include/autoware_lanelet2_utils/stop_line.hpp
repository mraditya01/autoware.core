 ...
 * @return Return either first stop line, stop line with target ID, or std::nullopt if no stop line
 * exists.
 */
std::optional<lanelet::ConstLineString3d> get_stop_lines_from_no_stopping_area(
  const lanelet::ConstLanelets & lanelets,
  const std::optional<lanelet::Id> & target_id = std::nullopt);
 ...
 * lanelets, optionally checking for a matching ID.
 *
