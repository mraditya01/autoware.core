...
lanelet::ConstLineString3d interpolate_linestring(const lanelet::ConstLineString3d &linestring,
  ...
  if (from_first) {
    for (std::size_t i = 0; i < linestring.size() - 1; ++i) {
      ...
      double segment_length = std::hypot(p2.x() - p1.x(),
                                         p2.y() - p1.y(),
                                         p2.z() - p1.z());
    ...
    lanelet::ConstPoint3d interp_pt = interpolate_point(linestring[segment_idx], linestring[segment_idx + 1], residue, true);
    ...
    double residue = distance - accumulated_length;
    lanelet::ConstPoint3d interp_pt = interpolate_point(linestring[segment_idx], linestring[segment_idx + 1], residue, false);
