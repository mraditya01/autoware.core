...
TEST_F(TestGetStopLineFromMap, GetStopLineFromNoStoppingArea_TargetFound)
{
  ...
  EXPECT_EQ(stopline_opt->id(), 208) << "Stop line ID does not match target 208 in NoStoppingArea.";
}
...
TEST_F(TestGetStopLineFromMap, GetStopLineFromDetectionArea_TargetFound)
{
  ...
  EXPECT_EQ(stopline_opt->id(), 209) << "Stop line ID does not match target 209 in DetectionArea.";
}
