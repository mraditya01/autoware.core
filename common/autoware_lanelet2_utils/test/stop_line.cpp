...
// test 1: get_stop_line_from_deprecated_crosswalk retrieves the stop line for a specific lane (ID
// 1993) from the map, then verifies that the stop line exists and has the expected ID.
TEST_F(TestGetStopLineFromMap, GetStopLineFromMap)
{
...
// test 2: get_stop_lines_from_no_stopping_area
TEST_F(TestGetStopLineFromMap, GetStopLinesFromNoStoppingArea)
{
...
// test 3: get_stop_lines_from_detection_area
TEST_F(TestGetStopLineFromMap, GetStopLinesFromDetectionArea)
{
  const lanelet::ConstLanelets lanelets(
    lanelet_map_regulatory_elements_ptr_->laneletLayer.begin(),
    lanelet_map_regulatory_elements_ptr_->laneletLayer.end());
...
// test 4: get_stop_lines_from_intersection_marking
TEST_F(TestGetStopLineFromMap, GetStopLinesFromIntersectionMarking)
{
  const lanelet::ConstLanelets lanelets(
    lanelet_map_regulatory_elements_ptr_->laneletLayer.begin(),
    lanelet_map_regulatory_elements_ptr_->laneletLayer.end());

  auto stoplines = lanelet2_utils::get_stop_line_from_intersection_marking(lanelets);

  ASSERT_FALSE(stoplines.empty()) << "No stop lines were found for DetectionArea.";

  std::set<lanelet::Id> unique_ids;
  for (const auto & sl : stoplines) {
    std::cout << "Stop line ID: " << sl.id() << std::endl;
    unique_ids.insert(sl.id());
  }
  EXPECT_EQ(stoplines.size(), unique_ids.size())
    << "Duplicate stop lines found in DetectionArea output.";
